...
def get_tifs_volume_shape(in_path, images_format = '.tif'):
    """
    ...
    first_file_path = os.path.join(in_path, tif_files[0])
    try:
        size_y = np.array(Image.open(first_file_path)).shape[0]
        size_x = np.array(Image.open(first_file_path)).shape[1]
...
if __name__ == "__main__":
    ...
    src_volume_path          = sys.argv[1] #"/run/media/leo/Home/eltech_redacted/60kv/card1"#sys.argv[1]
    info_path                = sys.argv[2] #"/run/media/leo/Home/nci/card/transformation_SITK.npy" #sys.argv[2]
    transformed_volumes_path = sys.argv[3] #"/home/leo/ImReg/day51/stitching_results/sitkMOM/markup_transformed_0"#sys.argv[3]
    ...
    src_volume = load_tifs_volume(src_volume_path)
    matrix = np.load(matrix_path)

    # src_volume = load_tifs_volume("C:/Leonid/volume_stitching/markups/wallnut")
    # matrix = np.array([[1,0,0,0],[0,0,-1,100],[0,1,0,0]])
    # transformed_volumes_path1 = "C:/Leonid/volume_stitching/markups/wallnut1"
    # transformed_volumes_path2 = "C:/Leonid/volume_stitching/markups/wallnut2"

    #print(f'Affine matrix for transformed volume = \n{matrix}')
    ...
    vedo_volume = vedo.Volume(src_volume)
    # tools.show_3d_vedo_volume(src_volume)
    del src_volume
    vedo_volume.permute_axes(2,1,0)
    try:
    ...
    for i in range(n_chunks):
        ...
        # if i == n_chunks - 1:
        #     print(f"origin = {[-origin_offset[0],-origin_offset[1],-origin_offset[2]+i*slices_per_chunk]}")
        #     print(f"dims = {np.array([new_volume_shape[0],new_volume_shape[1],new_volume_shape[2]-i*slices_per_chunk])}")
        # else:
        #     print(f"origin = {[-origin_offset[0],-origin_offset[1],-origin_offset[2]+i*slices_per_chunk]}")
        #     print(f"dims = {np.array([new_volume_shape[0],new_volume_shape[1],slices_per_chunk])}")
    ...
    #tools.show_3d_volume(general_volume)

    # #without memory economy
    # vedo_volume_new = vedo.Volume(input_obj=None, origin = [0,0,0], dims = new_volume_shape)
    # #del empty_arr
    # print("empty_ok")
    # vedo_volume_new.resample_data_from(vedo_volume)
    # vedo_volume = vedo_volume_new
    ...
    # print("new_ok")
    # vedo_volume.permute_axes(2,1,0)
    # save_volume = vedo_volume.tonumpy().astype(np.float32)
    # del vedo_volume
    # print("vedo2_ok")
    # #tools.show_3d_volume(save_volume)
    # print(general_volume.shape)
    # print(save_volume.shape)
    # print(metrics.mse(general_volume, save_volume))
    # print(metrics.mse(load_tifs_volume(transformed_volumes_path1), load_tifs_volume(transformed_volumes_path2)))
    # for i, v_slice in enumerate(save_volume):
    #     save_path = os.path.join(transformed_volumes_path2, str("%04d"%i) + '.tif')#slice_{i:05d}
    #     cv2.imwrite(save_path, v_slice)
