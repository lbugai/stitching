...
def get_paths_images(path, files_number, images_format = ".tif"):
    """
    Returns path to files with given template names.
    """
    file_paths = []
    for img_number in range(files_number):
        file_paths.append(f'{path}/{img_number}/{images_format}') # 0, 1, ...
        #file_paths.append(f'{path}/rec/{img_number:03d}/{images_format}')# 0001, 0002, ...
        #file_paths.append(path + "PRJ" + str(i) + images_format) # PRJ0, PRJ1, ...
    return file_paths
            ...
            for j in range(repeat):
                ...
                try:
                    ...
                    if alg_out["output"] == 0:
                        ...
                        if calculate_metrics :
                            metric_volume = load_tifs_volume(markup_path)
                ...
                x_shape, y_shape, z_shape = get_tifs_volume_shape(markup_path)[::-1]
                if registered_volumes_writing:
                    ...
                    if result.stderr:
                        ...
                        continue
                    try:
                        test_transformed_volume = load_tifs_volume(f'{sample_result_folder_path}/test_transformed_{j}')
                        if minimize_padding == True:
                            ...
                            markup_shape = get_tifs_volume_shape(markup_path)
                            target_shape = np.maximum(get_tifs_volume_shape(f'{sample_result_folder_path}/test_transformed_{j}'), np.array([markup_shape[0]+origin_offset[2],markup_shape[1]+origin_offset[1],markup_shape[2]+origin_offset[0]]))
                    ...
                    try:
                        markup_volume = load_tifs_volume(markup_path)
                        if minimize_padding == True:
