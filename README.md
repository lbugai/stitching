...
Есть несколько доступных режимов:
...
`MATRIX` - начальное преобразование задается матрицей в файле .json, этот способ был описан в предыдущем разделе `Работа с прогонщиком`;
...
Рекомендации: если ваши совмещаемые объемы слабо отличаются ориентацией (отличие порядка 30 градусов), то лучше использовать `MOMENTS`; режим `GEOMETRY` нельзя использовать, если объемы сильно отличаются ориентацией, например, если границы объекта в первом объеме в пределах 0-100 пикселей по одной из осей, а границы второго объема в пределах 500-600 пикселей, метод совместит центры изображений, но объекты в них могут не пересечься и метод даст некорректный результат, в целом лучше его никогда не использовать; метод `POINTS` возможно один из лучших режимов на сложных данных, но требует некоторых доп. настроек (описание будет добавлено позже), режим 'MATRIX' удобен при использовании на сложных данных, если начальное преобразование ищется в таком ПО как, например, `Sicer 3D` - здесь есть возможность получить преобразование сразу в виде матрицы.
