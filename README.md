...
Есть несколько доступных режимов: `GEOMETRY` - геометрический центр одного трехмерного изображения приводится к геометрическому центру другого изображения, только преобразование смещения; `MOMENTS` - центр масс одного трехмерного изображения приводится к центру масс другого изображения, только преобразование смещения; `POINTS` - режим на основе ручной разметки особых точек; `MATRIX` - начальное преобразование задается матрицей в файле .json, этот способ был описан в предыдущем разделе `Работа с прогонщиком`; `MANUAL` - ручная тонкая настройка преобразования в конфиге, позволяет отдельно задать масштаб, вращение (в порядке `xyz`), смещение (в порядке `xyz`), причем смещение задается либо вручную отдельно по каждой координате, либо с помощью описанных выше режимов `GEOMETRY` и `MOMENTS`.
