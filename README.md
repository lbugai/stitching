...
В основе инструмента лежит использование открытой библиотеки [SITK](https://simpleitk.org/) для поиска преобразования подобия (или аффинного), приводящего два 3D изображения в общие координаты.
...
`Важно!` В случае Arch Linux можно использовать данную инструкцию, отличие присутствует только в консольной команде активации виртуального окружения (пункт 2).
</details>
...
```bash
path\to\venv_name\bin\activate.csh
```
...
#### Выбор типа пространственного преобразования для поиска алгоритмом SITK.
В начале необходимо выбрать класс пространственного преобразования, которое будет искать SITK для совмещения исходных объёмов. Это позволяет ограничить степени свободы при оптимизации параметров преобразования алгоритмом.

В текущей версии доступны два типа:
...
- `Affine3D` - аффинное преобразование трёхмерного пространства с 12 степенями свободы: 3 - анизотропное масштабирование вдоль всех направлений, 3 - вращательные, 3 - сдвиговые и 3 - параллельный перенос.
...
Для использования этого режима необходимо в файле конфига метода установить параметру `InitalTransform` значение `POINTS`, а также указать параметру `imagej_landmark_coords_file_path` в качестве значения путь до файла с разметкой.
