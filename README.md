...
Из папки `runner` в консоли запустите прогон эксперимента `№0` (предварительно в конфиге метода `sitk_config.json` в поле `InitalTransform` необходимо указать значение `MATRIX`)
